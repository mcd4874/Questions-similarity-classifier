...
QUESTION_PAIRS_FILE = 'train_questions.csv'
...
if exists(Q1_TRAINING_DATA_FILE) and exists(Q2_TRAINING_DATA_FILE) and exists(LABEL_TRAINING_DATA_FILE) and exists(
    ...
    train_df = load_data(KERAS_DATASETS_DIR +QUESTION_PAIRS_FILE)[:1000]
    print('Question pairs: %d' % len(train_df))
    question1 = train_df['question1']
    question2 = train_df['question2']
...
test_predict[(test_predict>0.5) | (test_predict==0.5)] = 1
print(test_predict)
y_test = np.array(y_test)
print(y_test)
print(classification_report(test_predict,y_test))
# generate_report(model,[Q1_test, Q2_test],y_test)
